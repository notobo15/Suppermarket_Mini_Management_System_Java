...
public class DAO_ImportDetail {
	...
	public ArrayList<DTO_ImportDetail> findAll() throws SQLException {
		...
		try {
			PreparedStatement ptm = con.getConnection()
					.prepareStatement("SELECT * FROM import_detail WHERE status = 1");
			rs = ptm.executeQuery();
			while (rs.next()) {
				int id = rs.getInt("id");
	...
	public DTO_ImportDetail findById(int id) throws SQLException {
		...
		try {
			PreparedStatement ptm = con.getConnection()
					.prepareStatement("SELECT * FROM import_detail WHERE id = ? and status = 1;");
			ptm.setInt(1, id);
			rs = ptm.executeQuery();
			while (rs.next()) {
				int importId = rs.getInt("import_id");
	...
	public boolean create(DTO_ImportDetail newObj) throws SQLException {
		ConnectDB con = new ConnectDB();
		try {
			String query = "INSERT INTO import_detail(import_id, product_id, quantity, price)" + "VALUES (?, ?, ?, ?);";
	...
	public boolean updateById(DTO_ImportDetail newObj) throws SQLException {
		ConnectDB con = new ConnectDB();
		try {
			String query = "UPDATE account SET import_id = ?, product_id = ?, quantity = ?, price = ?  where id = ? and status = 1";
			PreparedStatement ptm = con.getConnection().prepareStatement(query);
			ptm.setInt(1, newObj.getImportId());
			ptm.setInt(2, newObj.getProductId());
			ptm.setFloat(3, newObj.getQuanity());
	...
	public boolean deteleById(int id) throws SQLException {
		...
		try {
			PreparedStatement ptm = con.getConnection()
					.prepareStatement("UPDATE import_detail SET status = 0 WHERE id = ?");
			ptm.setInt(1, id);
	...
	public boolean unDeteleById(int id) {
		...
		try {
			PreparedStatement ptm = con.getConnection()
					.prepareStatement("UPDATE import_detail SET status = 1 WHERE id = ?");
			ptm.setInt(1, id);
			rs = ptm.executeUpdate();
